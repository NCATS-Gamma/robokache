...
func (m *MockClient) Get(url string) (*http.Response, error) {
	if url != "https://qgraph.us.auth0.com/.well-known/jwks.json" {
		return &http.Response{
			StatusCode: 404,
			Body:       io.NopCloser(bytes.NewReader([]byte("Page not found"))),
		}, nil
	}
	cert, err := os.ReadFile(pubKeyPath)
	fatal(err)
	sub_cert := strings.Replace(string(cert), "-----BEGIN CERTIFICATE-----", "", 1)
	sub_cert = strings.Replace(sub_cert, "-----END CERTIFICATE-----", "", 1)
	json := fmt.Sprintf(`{"keys":[{"kid":"default","x5c":["%s"]}]}`, strings.ReplaceAll(sub_cert, "\n", ""))
...
func init() {
	...
	type MyCustomClaims struct {
		Email string `json:"https://qgraph.org/email,omitempty"`
		jwt.StandardClaims
	}
	...
	claims := MyCustomClaims{
		"me@robokache.com",
		jwt.StandardClaims{
			Issuer:   "https://qgraph.us.auth0.com/",
			Audience: []string{"https://qgraph.org/api"},
		},
	}
